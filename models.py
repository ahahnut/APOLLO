...
class MISA(nn.Module):
    def __init__(self, config):
        ...
        # shared encoder
        ##########################################
        self.shared_v = nn.Sequential()
        self.shared_v.add_module('shared_1', nn.Linear(in_features=config.hidden_size, out_features=config.hidden_size))
        self.shared_v.add_module('shared_activation_1', nn.Sigmoid())

        self.shared_a = nn.Sequential()
        self.shared_a.add_module('shared_1', nn.Linear(in_features=config.hidden_size, out_features=config.hidden_size))
        self.shared_a.add_module('shared_activation_1', nn.Sigmoid())

        self.shared_t = nn.Sequential()
        self.shared_t.add_module('shared_1', nn.Linear(in_features=config.hidden_size, out_features=config.hidden_size))
        self.shared_t.add_module('shared_activation_1', nn.Sigmoid())
    ...
    def alignment(self, sentences, visual, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask):
        ...
        #Privacy degree 
        epsilon = self.config.epsilon_noise

        #Correlation Discount Calculation
        correlation_dis_t = torch.mean(torch.cosine_similarity(self.utt_shared_t, self.utt_shared_v)) + torch.mean(torch.cosine_similarity(self.utt_shared_t, self.utt_shared_a))
        correlation_dis_v = torch.mean(torch.cosine_similarity(self.utt_shared_v, self.utt_shared_a)) + torch.mean(torch.cosine_similarity(self.utt_shared_v, self.utt_shared_t))
        correlation_dis_a = torch.mean(torch.cosine_similarity(self.utt_shared_a, self.utt_shared_v)) + torch.mean(torch.cosine_similarity(self.utt_shared_a, self.utt_shared_t))

        # Maxminize Correlation
        correlation_max = max(correlation_dis_t, correlation_dis_v, correlation_dis_a)

        #Global Sensitivity
        sensitivity_t = torch.mean(self.utt_shared_t)
        sensitivity_v = torch.mean(self.utt_shared_v)
        sensitivity_a = torch.mean(self.utt_shared_a)

        

        # Maximinize 
        sensitivity_max = max(sensitivity_t, sensitivity_v, sensitivity_a)

        #Noise degree
        degree_s_t = ( sensitivity_max/(epsilon * (1/sensitivity_max)) * (correlation_max + 1) ).cpu().detach().numpy()
        degree_s_v = ( sensitivity_max/(epsilon * (1/sensitivity_max)) * (correlation_max + 1) ).cpu().detach().numpy()
        degree_s_a = ( sensitivity_max/(epsilon * (1/sensitivity_max)) * (correlation_max + 1) ).cpu().detach().numpy()

        #Size
        size_s_t = self.utt_shared_t.size()
        size_s_v = self.utt_shared_v.size()
        size_s_a = self.utt_shared_a.size()


        #Laplace Noise
        noise_s_t = torch.from_numpy(np.random.laplace(0, degree_s_t, size_s_t)).cuda().float()
        noise_s_v = torch.from_numpy(np.random.laplace(0, degree_s_v, size_s_v)).cuda().float()
        noise_s_a = torch.from_numpy(np.random.laplace(0, degree_s_a, size_s_a)).cuda().float()


        # Plus Laplace Noise
        self.utt_shared_t = torch.add(self.utt_shared_t, noise_s_t)
        self.utt_shared_v = torch.add(self.utt_shared_v, noise_s_v)
        self.utt_shared_a = torch.add(self.utt_shared_a, noise_s_a)
    ...
    def shared_private(self, utterance_t, utterance_v, utterance_a):
        ...
        self.utt_shared_t = self.shared_t(utterance_t)
        self.utt_shared_v = self.shared_v(utterance_v)
        self.utt_shared_a = self.shared_a(utterance_a)
